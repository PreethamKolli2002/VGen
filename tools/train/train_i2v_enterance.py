...
def worker(gpu, cfg):
    ...
    if cfg.max_frames == 1:
        ...
        dataset = DATASETS.build(cfg.vid_dataset, sample_fps=cfg.sample_fps, transforms=train_trans, vit_transforms=vit_trans, max_frames=cfg.max_frames,get_first_frame=True)
    ...
    for step in range(resume_step, cfg.num_steps + 1): 
        ...
        # preprocess
        with torch.no_grad():
            y_image, _, y_words = clip_encoder(text=captions, image=vit_frame)
            ...
            try:
                ...
                pass
        # image_id_tensor = train_trans([ref_frame]).to(gpu)
        ...
        model_kwargs = {'y': y_words, 'image':y_image, 'local_image':local_image, 'fps': fps_tensor}
        if cfg.use_fsdp:
                ...
                try:
                    visual_kwards = [
                        ...
                        },
                        {
                            ...
                            'image':zero_feature.repeat(y_image_0.size(0), 1, 1),
                            'local_image':local_image[:viz_num],
                            'fps': fps_tensor[:viz_num]
                        }
                    ]
